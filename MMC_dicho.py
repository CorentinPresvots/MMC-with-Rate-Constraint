...
class Encode_one_window(Model_Encoder, Residual_Encoder):
    ...
    def f(self,id_model,nx,n_max):
        ...
        for id_residual_test in self.best_Residual_used :   
            ...
            nr=n_max-self.nm-self.best_Model_used[id_model]["n kx"]-self.best_Model_used[id_model]["n nx"]-nx-self.nl-self.best_Residual_used[id_residual_test]["n kr"]-self.best_Residual_used[id_residual_test]["n nr"]
    ...
    def MMC_enc(self, x, x_previous, n_max):
        ...
        # Step 4: Initialize variables to track the best signal-to-noise ratio (SNR) and quality
        SNR_tot = -np.inf
        ...
        for id_model_test in self.best_Model_used:
            ...
            if self.best_Model_used[id_model_test]["name"] == "none":
                ...
                # Otherwise, compute the results for the second test bitrate
                SNR_model_d, SNR_residual_d, theta_tilde_d, code_model_d, x_model_d, id_residual_d, \
                    kr_d, nb_sym_d, code_residual_d, x_residual_d = self.f(id_model_test, n_xd, n_max)
    
            # Step 7: Refine the bit allocation until the interval is small enough
            while True:
                if SNR_model_c + SNR_residual_c < SNR_model_d + SNR_residual_d:
                    # If the combined SNR at n_xc is worse, shift the interval to the right
                    min_rate_per_parameter = n_xc
                    if SNR_model_d + SNR_residual_d > SNR_tot:
                        # Update the best model and residual if this configuration is better
                        SNR_tot = SNR_model_d + SNR_residual_d
                        self.best_Model_used[id_model_test].update({
                            "theta tilde": theta_tilde_d,
                            "code model": code_model_d,
                            "x model": x_model_d,
                            "quality model": SNR_model_d,
                            "nx": n_xd,
                    ...
                    SNR_model_c, SNR_residual_c = SNR_model_d, SNR_residual_d
                    SNR_model_d, SNR_residual_d, theta_tilde_d, code_model_d, x_model_d, id_residual_d, \
                        kr_d, nb_sym_d, code_residual_d, x_residual_d = self.f(id_model_test, n_xd, n_max)
                else:
                    # If the combined SNR at n_xd is worse, shift the interval to the left
                    max_rate_per_parameter = n_xd
                    if SNR_model_c + SNR_residual_c > SNR_tot:
                        # Update the best model and residual if this configuration is better
                        SNR_tot = SNR_model_c + SNR_residual_c
                        self.best_Model_used[id_model_test].update({
                            "theta tilde": theta_tilde_c,
                            "code model": code_model_c,
                            "x model": x_model_c,
                            "quality model": SNR_model_c,
                            "nx": n_xc,
                    ...
                    # Shift values for the next iteration
                    SNR_model_d, SNR_residual_d = SNR_model_c, SNR_residual_c
                    SNR_model_c, SNR_residual_c, theta_tilde_c, code_model_c, x_model_c, id_residual_c, \
                        kr_c, nb_sym_c, code_residual_c, x_residual_c = self.f(id_model_test, n_xc, n_max)
