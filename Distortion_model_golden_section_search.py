...
class Encode_one_window(Model_Encoder, Residual_Encoder):
    ...
    def MMC_enc(self, x, x_previous, n_max):
        """
        Perform Multi-Model Compression (MMC) encoding. Distortion model folowed by golden section search
        ...
        for id_model_test in id_models:
            ...
            if n_xc == n_xd:
                # If n_xc = n_xd the second test point uses the same results as the first
            ...
            # Refine the bit allocation until the interval is small enough
            while True:
