...
class Encode_one_window(Model_Encoder, Residual_Encoder):
    def __init__(self, fn=50, fs=6400, N=128, Model_used={}, Residual_used={}):
        ...
        self.n_kr = 4  # Number of bits to encode the normalization factor `kr`, or 0 if no residual encoding is used.
    ...
    def MMC_enc(self, x, x_previous, n_max):
        """
        Perform Multi-Model Compression (MMC) encoding.
        ...
        for id_model in id_models:
            ...
            """
            #print("np.min([self.nb_max_bit_theta*len(self.best_Model_used[id_model][w theta]),2**self.best_Model_used[id_model][n nx]-1])",np.min([self.nb_max_bit_theta*len(self.best_Model_used[id_model]["w theta"]),2**self.best_Model_used[id_model]["n nx"]-1]))
            for n_x in Nx:
                ## First Stage: Model Encoding ##
                
                # Ensure `nx` is within valid bounds for the current model.
                #if n_x >= self.nb_min_bit_theta * len(self.best_Model_used[id_model]["m theta"]) and n_x <= 2**self.best_Model_used[id_model]["n nx"] - 1:
    
                # Encode the model parameters (`theta`) and reconstruct the model signal.
                ...
                ## Second Stage: Residual Encoding ##
                
                # Compute the residual signal as the difference between the original and modeled signals.
                r = self.best_Model_used[id_model]["xn"] - x_model
                
                SNR_residual = -1  # Initialize the best residual SNR for this model.

                for id_residual_test in self.best_Residual_used:
                    # Calculate the number of bits available for residual encoding (`nr`).
                    n_r = (
                        n_max 
                        - self.nm 
                    ...
                    if n_r >= 0:
                        # Encode the residual signal and compute its reconstruction.
                        x_residual_test, code_residual_test, kr_test, nb_sym_test = self.enc_residual(id_residual_test, r, n_r)
                        SNR_residual_test = -get_quality(r, x_residual_test, "SNR")

                        n_r_reel = len(code_residual_test)  # Actual number of bits used for residual encoding.

                        # Update the best residual configuration if the current SNR is better.
                        ...
                        if SNR_model + SNR_residual_test > SNR_tot:
                            SNR_tot = SNR_model + SNR_residual_test
                            ...
                            # Store the best residual configuration.
                            self.best_Model_used[id_model]["id residual"] = id_residual_test
                            self.best_Model_used[id_model]["name residual"] = self.best_Residual_used[id_residual_test]["name"]
                            self.best_Model_used[id_model]["x residual"] = x_residual_test

                            self.best_Model_used[id_model]["nb sym residual"] = nb_sym_test
                            self.best_Model_used[id_model]["nr"] = n_r_reel
                            self.best_Model_used[id_model]["code residual"] = code_residual_test

                            self.best_Residual_used[id_residual_test]["kr"] = kr_test

                            # Record the identifiers for the best model and residual methods.
                            self.best_model_used = id_model
                            self.best_residual_used = id_residual_test
...
class Decode_one_window(Model_Decoder, Residual_Decoder):
    ...
    def __init__(self, fn=50, fs=6400, N=128, Model_used={}, Residual_used={}):
        ...
        self.n_kr = 4  # Bits for kr encoding.
