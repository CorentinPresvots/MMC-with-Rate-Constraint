...
class Antonini_Encoder(Context_Aritmetic_Encoder):
    ...
    def get_symbol_second(self):
        ...
        for i in range(self.N):# on balaye tous les flags
            if self.flag[i]==0 :
                if np.abs(self.res[i])>=self.threshold: # le symbole est significatif
                    ...
                    if len(self.code)+len(code_R)+len(code_second)+self.follow+2<=self.br-self.nb_bits_coefs_max and self.nb_coefs<self.nb_coefs_max :
                        #### mise à jours des variables 
        ...
        if count>0:
            ...
            if len(self.code)+len(code_second)+self.follow+2<=self.br-self.nb_bits_coefs_max and self.nb_coefs<self.nb_coefs_max :
                #### mise à jours des variables 
        ...
        for char in test_symbol_second:
            if char == 'R':
                count += 1
            else:
                if count > 0:
                    ...
                    symbol_second.extend(count_bin[::-1])
                    count = 0
                symbol_second.append(char)

        if count > 0:
            symbol_second.append('E') 
...
if __name__ == "__main__":
    ...
    print('nombre de symboles codés "R,S,+,-E":',nb_coefs)
    #print(AE.symbol)
